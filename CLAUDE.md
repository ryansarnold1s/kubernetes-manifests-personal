...
- **V+ ≥10 is BepInEx-bound like every other mod here** — `BepInEx/config/org.bepinex.plugins.valheim_plus.cfg`, `Key = Value`, created by V+ on first boot (`Azumatt.*` / `blacks7ar.*` are BepInEx-bound too, with `On`/`Off` Toggle enums). A legacy `valheim_plus.cfg` (V+ ≤9.x's own INI file) beside it **overrides it on every launch** and makes settings read-only — never create one, by hand or via `MOD_CONFIG`. A rejected BepInEx value logs `could not be parsed` in the **game** container; whether a rejected V+ 10 value does too is **unverified**, so that check is not a gate for V+ — verify V+ by reading the value back off the PVC
